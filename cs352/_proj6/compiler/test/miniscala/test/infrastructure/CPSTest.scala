 ...
 */
abstract class CPSTest[CPSModule <: CPSTreeModule](val CPS: CPSModule) extends CompilerTest with BasicTest {
  ...
  def splitContinuations: Boolean
  ...
  object CPSVariableRenamePhase extends (CPS.Tree => CPS.Tree) {
    ...
    private[this] def updateTree(tree: Tree): Tree = (tree: @unchecked) match {
      ...
      case LetC(h +: t, body) if t.length > 0 && splitContinuations =>
        updateTree(LetC(Seq(h), LetC(t, body)))
