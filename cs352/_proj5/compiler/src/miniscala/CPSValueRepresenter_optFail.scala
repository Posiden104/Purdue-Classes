...
object CPSValueRepresenter_optFail extends (H.Tree => L.Tree) {
    ...
    case H.LetF(funs: Seq[H.FunDef], body) =>
      ...
      val workerDefs = funs map { f =>
        ...
        val w = Symbol.fresh("w_" + f.name);
				val s = Symbol.fresh("s_" +f.name);
        val env = Symbol.fresh("env_" + f.name);
				val sc = Symbol.fresh("sc_" + f.name);
        ...
        funNames = funNames.patch(funPtr, Seq(Seq(f.name, s)), 1);
        ...
        val subBody = f.body.subst(sub);
				...
				// create appF for worker
				val appF = L.AppF(w, f.retC, f.args ++ fv_names);

        // wrap free vars with appF for worker
        val res = wrap(fv_names zipWithIndex, appF) {
          case ((e, n), inner) =>
            tempLetL(n + 1) { c =>
              L.LetP(e, CPSBlockGet, Seq(env, c), inner)
				...
				// set up s and w fundefs
				val transSubBody = transform(subBody);
				val wFun = L.FunDef(w, f.retC, f.args ++ fv_names, transSubBody);
				val sFun = L.FunDef(s, sc, env +: f.args, res);
        ...
        (wFun, sFun)
			...
			// unzip the workers
			val unzippedWorkerDefs = workerDefs.unzip._1 ++ workerDefs.unzip._2;
      ...
      L.LetF(unzippedWorkerDefs, wrappedAlloc)
