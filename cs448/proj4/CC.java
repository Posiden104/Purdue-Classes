...
public class CC
{
	/* Joel Van Auken
	 * jvanauke
	 * CS448 Project 4 - Concurrency Control
	 */
	...
	*/
	...
	private static boolean requestLock(Transaction t, LockType type, int recID){
		...
		boolean isInShared = shrdList.contains(t.priority);
		boolean sharedEmpty = shrdList.isEmpty();
		int ID = t.priority;
		int graphID = ID - 1;
		List<Integer> locGraph = graph.get(graphID);
		...
		if(lt == LockType.exclusive) {
			...
			if(!(locGraph.contains(excLocID))) locGraph.add(excLocID);
			return false;
		}
		...
		switch(type){
			...
			case exclusive:
				...
				// remove myself
				locGraph.remove(new Integer(ID));
...
class Transaction
{
	...
	public boolean nextOp(){
		...
		else if(++numRejected >= Math.max(CC.transList.size() * 2, 25)) CC.detectDeadlocks();
		return succ;
	}
